...
// Récupère le billet (réservation) pour l'utilisateur connecté.
$booking = db_single(
...
// QR code : on encode une URL vérifiable (id reservation).
$payload = '/pages/billet_verify.php?reservation_id=' . (int)$booking['reservation_id'];

// Génération QR code en mode sans dépendance :
// - si GD + bibliothèques QR ne sont pas dispo, on affiche une version texte.
// Ici on utilise une approche simple : on génère un <img> via Google Chart API.
// (En production, préférer une lib locale type chillerlan/php-qrcode.)
  ...
  <div style="margin-top:16px; padding:16px; background: rgba(255,255,255,.03); border:1px solid var(--border); border-radius:14px; max-width:820px;">
    <div style="display:grid; grid-template-columns: 280px 1fr; gap:16px; align-items:start;">
      ...
      <div>
        ...
        <p style="margin:0 0 6px; color: var(--muted);"><strong>Date :</strong> <?= e((new DateTime((string)$booking['date_event']))->format('d/m/Y H:i')) ?></p>
        <p style="margin:0 0 10px; color: var(--muted);"><strong>Réservation :</strong> #<?= (int)$booking['reservation_id'] ?></p>
        <p style="margin:0; color: var(--muted); font-size:13px;">Payload QR :</p>
        <div style="margin-top:6px; padding:10px; border-radius:12px; border:1px solid var(--border); background: rgba(255,255,255,.02); color: var(--text); word-break:break-all;"><?= e($payload) ?></div>
      </div>
    </div>
